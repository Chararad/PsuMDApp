#Region  Project Attributes
    #ApplicationLabel: Flash Leaderboard
    #VersionCode: 1
    #VersionName: 1.0
...
Private Sub SetupTheme
	'Lofi / cozy palette
	Activity.Color = xui.Color_ARGB(255, 245, 239, 229) 'warm cream

	pnlHeader.Color = xui.Color_ARGB(255, 107, 92, 141) 'soft violet
	lblTitle.TextColor = xui.Color_White
	lblSubtitle.TextColor = xui.Color_ARGB(255, 232, 224, 255)

	pnlTopCard.Color = xui.Color_ARGB(255, 255, 247, 236) 'paper card

	lblTopUser.TextColor = xui.Color_ARGB(255, 64, 52, 82)
	lblTopPoints.TextColor = xui.Color_ARGB(255, 105, 89, 128)
	lblTopStreak.TextColor = xui.Color_ARGB(255, 131, 102, 82)

	btnDaily.Color = xui.Color_ARGB(255, 185, 155, 214)
	btnWeekly.Color = xui.Color_ARGB(255, 185, 155, 214)
	btnAllTime.Color = xui.Color_ARGB(255, 185, 155, 214)

	btnDaily.TextColor = xui.Color_White
	btnWeekly.TextColor = xui.Color_White
	btnAllTime.TextColor = xui.Color_White

	lblFooter.TextColor = xui.Color_ARGB(255, 120, 104, 94)
End Sub

Private Sub SetupInsertControls
	pnlInsert.Initialize("")
	pnlInsert.Color = xui.Color_ARGB(255, 255, 247, 236)
	Activity.AddView(pnlInsert, 8dip, Activity.Height - 154dip, Activity.Width - 16dip, 146dip)

	edtName.Initialize("")
	edtName.Hint = "Name"
	pnlInsert.AddView(edtName, 8dip, 8dip, pnlInsert.Width - 16dip, 40dip)

	edtXP.Initialize("")
	edtXP.Hint = "XP"
	edtXP.InputType = edtXP.INPUT_TYPE_NUMBERS
	pnlInsert.AddView(edtXP, 8dip, 54dip, (pnlInsert.Width - 24dip) / 2, 40dip)

	edtStreak.Initialize("")
	edtStreak.Hint = "Streak"
	edtStreak.InputType = edtStreak.INPUT_TYPE_NUMBERS
	pnlInsert.AddView(edtStreak, edtXP.Left + edtXP.Width + 8dip, 54dip, (pnlInsert.Width - 24dip) / 2, 40dip)
	...
	pnlInsert.AddView(btnInsert, 8dip, 100dip, pnlInsert.Width - 16dip, 40dip)
End Sub
...
Private Sub ShowBoard(mode As String)
	...
	For i = 0 To source.Size - 1
		...
		Dim p As Panel
		p.Initialize("")
		p.Color = xui.Color_ARGB(255, 255, 250, 242)
...
Private Sub HighlightButton(activeBtn As Button)
	Dim activeColor As Int = xui.Color_ARGB(255, 141, 113, 176)
	Dim normalColor As Int = xui.Color_ARGB(255, 185, 155, 214)

	btnDaily.Color = normalColor
	btnWeekly.Color = normalColor
	btnAllTime.Color = normalColor
